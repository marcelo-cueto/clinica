...
class LoginController extends Controller
{
   ...
   function enviar_provisoria()
   {
      ...
      if ($_REQUEST['csrf_token'] == $_SESSION['csrf_token']['token']) {
         if (isset($_POST['email'])) {
            ...
            else $mensaje = 'No hay ning&uacute;n administrador registrado con este email';
         } else $mensaje = 'Falta completar datos';
   ...
   {
      if (isset($_SESSION['user'])) Redirect::to('profesionales');
      if (isset($_POST['nueva-clave'])) {
         if ($_POST['nueva-clave'] == $_POST['confirmacion-clave']) {
            $data = [
            ...
            Alert::throw_msg('Las claves no coinciden o falta completar informaci&oacute;n', 'danger');
         }
      }
      $data = ['id' => $id];
